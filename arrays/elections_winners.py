...
and an integer k equal to the number of voters who haven't cast their vote yet, f
ind the number of candidates who still have a chance to win the election.
...
Thus, only 2 candidates can win (the second and the third), whic
'''
...
def electionsWinners(votes, k):
    ...
    possibles = int(votes.count(leader) == 1) if k == 0 else len(
        [x for x in votes if x + k > leader])
